...
class CursosHasGruposMySqlDAO implements CursosHasGruposDAO{
	...
	/**
	* Devuelve el nombre del curso y la sesion
	*/
	public function getCursoYGrupoByUsuario($usuario_id){
		$sql = 'SELECT c.* FROM grupos as g, cursos as c, cursos_has_grupos AS cg'.
			' WHERE g.id IN( SELECT id_grupo FROM grupos_has_estudiantes'.
				' WHERE id_persona = ?)'.
			' AND cg.id_grupo = g.id AND cg.id_curso = c.id';
