...
class ContenidosMySqlDAO implements ContenidosDAO{
	...
	public function getContenidosByQuiz($id_quiz){
		$sql ='SELECT c.* 
                    FROM contenidos c 
                    JOIN preguntas p ON c.id=p.id_contenido 
                    JOIN quizes_has_categorias qc ON qc.id_categoria=p.id_categoria 
                    WHERE qc.id_quiz = ?
                    GROUP BY c.id';
            $sqlQuery = new SqlQuery($sql);
	...
	public function insert($contenido){
		...
		$sqlQuery->setString($contenido->nombre);
		$sqlQuery->setString($contenido->linkRepaso);
		$sqlQuery->setString($contenido->fraseNoLogrado);
		$sqlQuery->setString($contenido->fraseLogrado);
	...
	public function queryByContenidoPadre($value){
		$sql = 'SELECT * FROM contenidos WHERE padre = ?';
		$sqlQuery = new SqlQuery($sql);
